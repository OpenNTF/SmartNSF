...
import org.openntf.xrest.xsp.utils.LMBCSConverter;
...
public class Token {
	...
	public Token(String token, String tokenName, String tokenDomain) {
		m_LtpaToken = token;
		m_RawToken = Base64.getDecoder().decode(token);
	...
	public static Token generate(String canonicalUser, Date tokenCreation, TokenConfiguration config) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		md.update(token);
		ltpa.m_Digest = md.digest(java.util.Base64.getDecoder().decode(config.getSecret()));
		token = concatenate(token, ltpa.m_Digest);
		ltpa.m_RawToken = token;
		ltpa.m_LtpaToken = java.util.Base64.getEncoder().encodeToString(token);
